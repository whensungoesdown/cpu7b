...
vlog +incdir+${PROJECT_ROOT}/rtl -f "$TEMP_FLIST" ${PROJECT_ROOT}/tb/test38_ext_intr_on_2lsu_inst_random_top_tb.v
rm -f "$TEMP_FLIST"

# 为ModelSim生成不同的种子
# ModelSim需要在 vsim 命令中使用 -g 参数或 + 参数
SEED=$(( $(date +%s) + $$ ))
echo "Using seed: $SEED"

# 方法1：使用 + 参数传递（推荐）
#vsim -c -do "run -all; quit" top_tb -wlf result.wlf +seed=$SEED
vsim -c -do sim.do top_tb -wlf result.wlf +seed=$SEED

# 或者方法2：使用 -g 参数传递（如果需要）
# vsim -c -do "run -all; quit" top_tb -wlf result.wlf -gSEED=$SEED
